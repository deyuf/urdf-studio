// Vendored from xacro-parser@0.3.11 (src/XacroParser.js) with local patches
// applied to support ROS-flavoured xacro constructs that the upstream parser
// rejects (object property substitution, etc.). See applyXacroCompatibilityRewrites
// in ../../core/xacro.ts for text-level rewrites that complement these patches.
import {
    getUrlBase,
    ...
    PARENT_SCOPE,
} from 'xacro-parser/src/utils.js';
import { ExpressionParser } from 'xacro-parser/src/ExpressionParser.js';
            ...
            function unpackParams(str, properties) {
                ...
                const res = tokens.map(match => {
                    ...
                    if (isDictionaryAccessor) {
                        // PATCH: the prefix is either a bare identifier or an
                        // identifier-call like `load_yaml(config_file)`. Restricting
                        // the prefix this way prevents leading unary operators (`-`)
                        // from being swallowed into __read_property__'s first arg.
                        contents = contents.replace(/([A-Za-z_]\w*(?:\([^()]*\))?)(\[[^[]+\])+/g, (match, ...args) => {
                    ...
                    if (isRospackCommand) {
                        ...
                        // PATCH: track identifiers that resolve to non-primitive values
                        // (YAML dicts, arrays, etc.). Upstream stringifies these into
                        // "[object Object]", which breaks the expr-eval pass. Instead we
                        // leave the identifier in place and feed the *resolved* value
                        // into the evaluation scope below.
                        //
                        // Pre-scan all identifier-like tokens in `contents` (not just
                        // those that survive tokenization as standalone tokens) so we
                        // also handle cases like `__read_property__(inertials,name)`
                        // where the tokenizer keeps `inertials,name` glued together.
                        const objectOverrides = {};
                        // Strip quoted strings before scanning identifiers so we don't
                        // pick up substrings like `inertials` inside a file-path literal
                        // such as `'/path/to/inertials.yaml'` — that would cause a
                        // bogus recursive unpackParams call on the matching property.
                        const scanTarget = contents.replace(/(['"`])(?:\\.|(?!\1)[^\\])*\1/g, '');
                        const identifierRegex = /\b([A-Za-z_]\w*)\b/g;
                        let identifierMatch;
                        while ((identifierMatch = identifierRegex.exec(scanTarget)) !== null) {
                            const ident = identifierMatch[1];
                            if (ident in properties && !(ident in objectOverrides)) {
                                const resolved = unpackParams(properties[ident], properties);
                                if (resolved !== null && typeof resolved === 'object') {
                                    objectOverrides[ident] = resolved;
                                }
                            }
                        }
                        ...
                        if (isString(expr)) {
                            ...
                            const evalScope = Object.keys(objectOverrides).length > 0
                                ? { ...properties, ...objectOverrides }
                                : properties;
                            return expressionParser.evaluate(cleanExpr, evalScope);
                        }
                    }
                });
        ...
        async function processNode(node, properties, macros, resultsList = []) {
            ...
            switch (tagName) {
                ...
                case 'xacro:property': {
                    ...
                    // PATCH: honour `lazy_eval="false"` (ROS xacro standard) to force
                    // eager evaluation; without this, objects pulled from YAML are
                    // stored as literal expression strings and downstream member
                    // accesses like `link_inertials.origin.rpy` fail.
                    const lazyEval = node.getAttribute('lazy_eval');
                    const forceEager = lazyEval === 'false';

                    // Emulated behavior here
                    // https://github.com/ros/xacro/blob/melodic-devel/src/xacro/__init__.py#L565
                    if (scope !== 'local' || forceEager) {
