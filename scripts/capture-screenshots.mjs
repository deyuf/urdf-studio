// Refreshes the README's web screenshots in media/screenshots-web/ using
// the vendored Franka primitives fixture. Run after any editor / Checks
// panel UI changes.
//
...
function buildLoadRobotMessage() {
  const links = {};
  for (const m of urdf.matchAll(/<link\s+name="([^"]+)"/g)) links[m[1]] = { name: m[1], childJoints: [], line: 0 };
  const joints = {};
...
try {
  ...
  // 12 — Source editor split view.
  await page.locator('[data-tab="source"]').click();
  await page.waitForSelector('#panel-source .cm-editor');
  await page.waitForTimeout(500);
  await page.screenshot({ path: path.join(OUT_DIR, '12-editor-franka.png') });

  // 13 — Source editor fullscreen (F11 mode).
  await page.evaluate(() => {
    document.querySelector('#panel-source').dispatchEvent(new CustomEvent('urdf-studio:request-fullscreen-toggle', { bubbles: true }));
  });
  await page.waitForTimeout(500);
  await page.screenshot({ path: path.join(OUT_DIR, '13-editor-fullscreen.png') });
  ...
  // 14 — Checks panel showing health score.
  await page.locator('[data-tab="checks"]').click();
  await page.waitForSelector('#panel-checks .health-score');
  await page.waitForTimeout(300);
  await page.screenshot({ path: path.join(OUT_DIR, '14-checks-health.png') });

  console.log('Saved 12-editor-franka.png, 13-editor-fullscreen.png, 14-checks-health.png');
} finally {
  await browser.close();
  await server.close();
}
